...
//   // --- LOGIC: Koneksi Firebase, Notif, Getar & Suara ---
//   useEffect(() => {
//     // Izin Notifikasi Pop-up (Opsional)
...
//             // LOGIC EKSEKUSI (Hanya untuk ORTU)
//             if (newStatus !== "idle" && newName !== "-" && role === "ORTU") {
//                 // Pastikan data benar-benar berubah agar tidak spam suara/getar terus menerus
//                 if (newName !== currentName || newStatus !== currentStatus) {
                     
//                      // 1. FITUR GETAR (Vibration)
//                      // Pola: Getar 500ms, Diam 200ms, Getar 500ms
//                      if (typeof navigator !== "undefined" && navigator.vibrate) {
//                         navigator.vibrate([500, 200, 500]);
//                      }

//                      // 2. FITUR SUARA (Text-to-Speech)
//                      if (typeof window !== "undefined" && 'speechSynthesis' in window) {
//                         // Stop suara sebelumnya jika masih ngomong
//                         window.speechSynthesis.cancel(); 

//                         const textToRead = newStatus === 'bersiap' 
//                             ? `Mohon perhatian. Ananda ${newName}, harap bersiap.`
//                             : `Panggilan untuk Ananda ${newName}, silakan masuk.`;
...
//                      // 3. FITUR NOTIF POP-UP (Jika layar mati/background)
//                      if (document.hidden) { 
...
//     });
//     return () => unsubscribe();
//   }, [jenjang, kelas, role, activeCall]); // Dependencies penting
...
//       <p className="mt-12 text-xs text-slate-300 font-medium">v2.4 • Suara & Getar Aktif</p>
//     </div>
//   );
// }
...
export default function AppRapor() {
  ...
  const [jenjang, setJenjang] = useState("");
  const [kelas, setKelas] = useState("");
  const [loading, setLoading] = useState(true); // Biar gak kedip pas load storage
  ...
  // --- LOGIC 1: AUTO-RESTORE DARI LOCALSTORAGE (SOLUSI UX) ---
  useEffect(() => {
  ...
  const handleReset = () => {
    ...
    setJenjang("");
    setKelas("");
    // Role tidak dihapus total biar gak perlu login password lagi, kecuali logout total
  };
  ...
  // --- LOGIC 2: Koneksi Firebase & Notif ---
  useEffect(() => {
    if (typeof window !== "undefined" && "Notification" in window && Notification.permission !== "granted") {
      Notification.requestPermission();
    }
    ...
    const unsubscribe = onValue(dbRef, (snapshot) => {
      ...
      if (data) {
        if (data.active) {
            ...
            const newName = data.active.nama;
            const currentName = activeCall.nama;
            const currentStatus = activeCall.status; // status sebelumnya di state local
            
            // LOGIC NOTIFIKASI (Hanya untuk ORTU)
            if (newStatus !== "idle" && newName !== "-" && role === "ORTU") {
                // Cek jika data berubah DIBANDINGKAN data yg baru diambil dr firebase
                // Kita gunakan trik sederhana: jika nama beda ATAU status beda
                if (newName !== currentName || newStatus !== currentStatus) {
                     
                     // A. JIKA LAYAR AKTIF (User sedang melihat)
                     if (!document.hidden) {
                        triggerVibrateAndAudio(newName, newStatus);
                     } 
                     
                     // B. JIKA LAYAR MATI/BACKGROUND (Kirim Notif System)
                     else {
                        const title = newStatus === 'bersiap' ? "⚠️ PERSIAPAN!" : "📢 SILAKAN MASUK!";
                        const body = newStatus === 'bersiap' 
                            ? `Ananda ${newName} harap bersiap di ${data.active.pos}.` 
                        ...
                        if (Notification.permission === "granted") {
                           const notif = new Notification(title, { body: body, icon: "/icon.png", tag: "antrian" });
                           // Saat notif diklik, bunyikan suara ulang
                           notif.onclick = () => {
                                window.focus();
                                triggerVibrateAndAudio(newName, newStatus);
                                notif.close();
    ...
    return () => unsubscribe();
  }, [jenjang, kelas, role, activeCall]); // Dependency penting
  ...
  const triggerVibrateAndAudio = (name, status) => {
    ...
    if (typeof window !== "undefined" && 'speechSynthesis' in window) {
        window.speechSynthesis.cancel(); 
        const textToRead = status === 'bersiap' 
  ...
  const btnAsramaCeklis = () => {
    ...
    const currentStatus = progressSiswa[nama];
    let nextStatus = currentStatus === 'done_sekolah' ? 'selesai_total' : 'done_asrama';
    update(ref(db), { [`sekolah/${jenjang}/${kelas}/progress/${nama}`]: nextStatus });
    setSelectedSiswa(""); 
  };
  ...
  const getListSiswa = () => {
    const allSiswa = DATA_SEKOLAH[jenjang]?.[kelas] || [];
    if (role === 'GURU_SEKOLAH') return allSiswa.filter(n => progressSiswa[n] !== 'done_sekolah' && progressSiswa[n] !== 'selesai_total');
    if (role === 'GURU_ASRAMA') return allSiswa.filter(n => progressSiswa[n] !== 'done_asrama' && progressSiswa[n] !== 'selesai_total');
    return [];
  };

  // --- LOADING SCREEN (Biar UX Enak) ---
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-400">Memuat data...</div>;

  // --- RENDER ---
  // PENTING: Props saveState diturunkan ke komponen anak
  if (!role) return <MenuAwal setRole={setRole} saveState={saveState} />;
  if (!jenjang || !kelas) return <MenuKelas setJenjang={setJenjang} setKelas={setKelas} setRole={setRole} saveState={saveState} handleLogoutTotal={handleLogoutTotal} />;
      ...
      <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-800">
        <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
            <div className="bg-slate-900 p-6 text-white flex justify-between items-center">
                ...
                <button onClick={handleReset} className="text-xs bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-full transition">Ganti Kelas</button>
            </div>
            <div className="p-8 bg-gradient-to-br from-emerald-50 to-teal-50 border-b border-emerald-100 text-center">
                <div className="text-5xl font-extrabold text-emerald-600 mb-2">{selesaiTotal} <span className="text-2xl text-emerald-400 font-medium">/ {allSiswa.length}</span></div>
                <div className="text-sm font-bold text-emerald-700 uppercase tracking-widest">Siswa Selesai Total</div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {allSiswa.map((nama, idx) => {
                            const status = progressSiswa[nama];
                            const doneSekolah = ['done_sekolah','selesai_total'].includes(status);
                            const doneAsrama = ['done_asrama','selesai_total'].includes(status);
                            return (
                                <tr key={nama} className={idx%2===0?'bg-white':'bg-slate-50/50'}>
                                    <td className="px-6 py-4 font-medium">{nama}</td>
                                    <td className="px-6 py-4 text-center">{doneSekolah ? '✅' : '-'}</td>
                                    <td className="px-6 py-4 text-center">{doneAsrama ? '✅' : '-'}</td>
                                    <td className="px-6 py-4 text-center">{status==='selesai_total' ? <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-xs font-bold">TUNTAS</span> : '-'}</td>
                                </tr>
                            );
                        })}
                    </tbody>
  ...
  if (role.includes("GURU")) {
    ...
    return (
      <div className={`min-h-screen p-6 font-sans text-slate-800 ${isSekolah ? 'bg-gradient-to-br from-blue-50 to-indigo-100' : 'bg-gradient-to-br from-indigo-50 to-purple-100'}`}>
        <div className="max-w-lg mx-auto">
          <div className="flex justify-between items-center mb-8 bg-white/60 backdrop-blur-md p-4 rounded-2xl shadow-sm border border-white/40">
            <button onClick={handleReset} className="text-slate-500 hover:text-slate-800 text-sm font-medium flex items-center gap-1">← Ganti Kelas</button>
          ...
          <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-xl border border-white/50">
            ...
            {isSekolah ? (
                <div className="space-y-3">
                    ...
                    <button onClick={btnSekolahMasuk} disabled={!isActiveMatch && activeCall.status !== 'bersiap'} className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-200 disabled:text-slate-400 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-[0.98] transition">📢 PANGGIL: MASUK</button>
                    <div className="h-px bg-slate-100 my-4"></div>
  ...
  // 3. ORANG TUA (Passing reset handler)
  return <TampilanOrtu activeCall={activeCall} jenjang={jenjang} kelas={kelas} handleReset={handleReset} />;
}
...
function MenuAwal({ setRole, saveState }) {
  ...
  const handleMenuClick = (roleTujuan) => {
    if (roleTujuan === "ORTU" || roleTujuan === "MONITOR") {
        setRole(roleTujuan);
        saveState("rapor_role", roleTujuan); // SAVE ROLE
    } else {
        setTargetRole(roleTujuan);
        setShowAuth(true);
        setErrorMsg("");
  ...
  const handleLogin = (e) => {
    ...
    if ((targetRole === "GURU_SEKOLAH" && inputPass === "sekolah2025") || (targetRole === "GURU_ASRAMA" && inputPass === "asrama2025")) {
        setRole(targetRole);
        saveState("rapor_role", targetRole); // SAVE ROLE SETELAH LOGIN
    } else {
        setErrorMsg("Password salah!");
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50 p-6 font-sans text-slate-800 relative">
      ...
      <p className="mt-12 text-xs text-slate-300 font-medium">v3.0 • Auto Save & Notif Fix</p>
    </div>
  );
}

function MenuKelas({ setJenjang, setKelas, setRole, saveState, handleLogoutTotal }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 font-sans">
        <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md border border-slate-100 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-blue-500 to-indigo-500"></div>
          <button onClick={handleLogoutTotal} className="text-sm text-slate-400 hover:text-red-500 mb-6 flex items-center gap-1 transition">← Kembali ke Menu Utama</button>
          <h2 className="text-2xl font-bold mb-6 text-slate-800">Pilih Kelas</h2>
          <div className="space-y-4">
            <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Jenjang</label>
...
function TampilanOrtu({ activeCall, jenjang, kelas, handleReset }) {
  // Logic Styling Modern
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-6 transition-colors duration-700 font-sans ${bgClass}`}>
      <button onClick={handleReset} className="absolute top-6 left-6 text-xs bg-white/20 backdrop-blur-md px-4 py-2 rounded-full text-white/90 hover:bg-white/30 font-bold transition">← Ganti Kelas</button>
