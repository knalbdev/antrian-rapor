...
export default function AppRapor() {
  ...
  // --- HELPER: AUDIO (FIXED FOR INDONESIAN ACCENT) ---
  const speakIndonesian = (text, volume = 1.0) => {
    if (typeof window === "undefined" || !('speechSynthesis' in window)) return;
    ...
    // Cari suara Indonesia (Damayanti/Lainnya)
    const voices = window.speechSynthesis.getVoices();
    const indoVoice = voices.find(v => v.lang === 'id-ID' || v.lang === 'id_ID');
    if (indoVoice) u.voice = indoVoice;
...
function MenuAwal({ setRole, saveState }) {
  ...
  const handleMenuClick = (roleTujuan) => {
    speakIndonesian(" "); // Unlocking Audio
    if (roleTujuan === "ORTU" || roleTujuan === "MONITOR") {
        setRole(roleTujuan); saveState("rapor_role", roleTujuan);
    } else {
        setTargetRole(roleTujuan); setShowAuth(true); setErrorMsg(""); setInputPass(""); setShowPassword(false);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50 p-6 font-sans text-slate-800 relative">
      ...
      <p className="mt-12 text-xs text-slate-300 font-medium">v7.0 • Dropdown Dinamis</p>
    </div>
  );
}

// --- FIX: DROPDOWN DINAMIS (Berdasarkan Jenjang) ---
function MenuKelas({ setJenjang, setKelas, setRole, saveState, handleMenuUtama }) {
  const [selectedJenjang, setSelectedJenjang] = useState("");
  
  const pilihJenjang = (val) => { 
      setSelectedJenjang(val); 
      setJenjang(val); 
      saveState("rapor_jenjang", val); 
      setKelas(""); // Reset kelas jika jenjang berubah
  };
  
  const pilihKelas = (val) => { 
      setKelas(val); 
      saveState("rapor_kelas", val); 
  };

  // Ambil daftar kelas berdasarkan jenjang yang dipilih
  const listKelas = selectedJenjang ? Object.keys(DATA_SEKOLAH[selectedJenjang]) : [];
          ...
          <div className="space-y-4">
            ...
            <div>
                ...
                <div className="relative">
                    <select className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none" onChange={(e) => pilihJenjang(e.target.value)} value={selectedJenjang}>
